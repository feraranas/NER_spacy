...
@spacy.registry.misc("rel_get_instances.v1")
...
def create_instances(max_length: int) -> Callable[[Doc], List[Tuple[Span, Span]]]:
    ...
    return lambda doc: get_instances(max_length, doc)
...
def rel_probabilities(doc, 
                      nlp,
                      show_table: bool = True,
                      title: Optional[str] = "Relation probabilities",
                      threshold=0.5,
    ...
    st.text(f"Text: {doc.text}")
    st.text(f"Entities: {[(e.start, e.text, e.label_) for e in pred.ents]}")

    # Create a list to store relations above the threshold
    relations_above_threshold = []

    # Iterate through relations and print those above the threshold
    for value, rel_dict in pred._.rel.items():
        if rel_dict['KNOWS'] > threshold or rel_dict['HAS'] > threshold:
            ...
            relations_above_threshold.append({
                'value': value,
                'KNOWS_probability': rel_dict['KNOWS'],
                'HAS_probability': rel_dict['HAS'],
            })

    if show_table and relations_above_threshold:
        df_relations = pd.DataFrame(relations_above_threshold)
...
def visualize_ner(
    ...
    *,
    ...
    if not labels:
        ...
        with st.expander("Click to see the named entities in an structured format"):
            if show_table:
                data = [
